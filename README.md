...
Current local app shell is v1.13.0. The app identity is Bills, while ordinary UI terminology such as `Subscriptions`, `Add subscription`, `Upcoming payments`, `Spending overview`, `Activity`, and `Backup` remains unchanged by design. The GitHub repository and Pages URL have not been renamed yet; the live app remains at `https://chillin-spree.github.io/subscription-tracker/` until a dedicated migration pass. Existing localStorage keys keep their `subscription-tracker-v1-*` names for compatibility.
